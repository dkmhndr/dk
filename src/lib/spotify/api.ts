...
import type { SpotifyNowPlaying, SpotifyTopTracks } from './types';
...
const TOP_TRACKS_ENDPOINT = `https://api.spotify.com/v1/me/top/tracks?time_range=${TimeRange.LastMonth}&limit=20`;
...
export const fetchNowPlaying = async () => {
  try {
    ...
    const year = song.item.album.release_date.split('-')[0];
    const albumImageUrl = song.item.album.images[0].url;
    const songUrl = song.item.external_urls.spotify;
    const id = song.item.id;

    return { album, albumImageUrl, artist, isPlaying, songUrl, title, year, id };
  } catch (error) {
    console.error(error);
    return { isPlaying: false };
  }
